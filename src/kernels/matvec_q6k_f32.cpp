#include "ultima/kernels/dequant_q6k.hpp"
#include "ultima/kernels/matvec.hpp"

#include <cstddef>
#include <cstdint>
#include <cstring>
#include <immintrin.h>

namespace ultima::kernels {

namespace {

float fp16_to_float(std::uint16_t h) noexcept {
    const std::uint32_t sign = (h & 0x8000u) << 16;
    std::uint32_t exp  = (h & 0x7C00u) >> 10;
    std::uint32_t mant = (h & 0x03FFu);
    std::uint32_t out_bits;
    if (exp == 0) {
        if (mant == 0) {
            out_bits = sign;
        } else {
            unsigned shift = 0;
            while ((mant & 0x0400u) == 0) { mant <<= 1; ++shift; }
            mant &= 0x03FFu;
            exp   = 127u - 14u - shift + 1u;
            out_bits = sign | (exp << 23) | (mant << 13);
        }
    } else if (exp == 0x1Fu) {
        out_bits = sign | 0x7F800000u | (mant << 13);
    } else {
        exp = exp - 15u + 127u;
        out_bits = sign | (exp << 23) | (mant << 13);
    }
    float f;
    std::memcpy(&f, &out_bits, sizeof(f));
    return f;
}

inline float read_fp16_le(const std::uint8_t* p) noexcept {
    const std::uint16_t h = static_cast<std::uint16_t>(p[0])
                          | (static_cast<std::uint16_t>(p[1]) << 8);
    return fp16_to_float(h);
}

inline float hsum256(__m256 v) noexcept {
    __m128 lo = _mm256_castps256_ps128(v);
    __m128 hi = _mm256_extractf128_ps(v, 1);
    __m128 s  = _mm_add_ps(lo, hi);
    s = _mm_hadd_ps(s, s);
    s = _mm_hadd_ps(s, s);
    return _mm_cvtss_f32(s);
}

} // namespace

void matvec_q6k_f32_scalar(const std::uint8_t* w, const float* x, float* y,
                           std::size_t M, std::size_t K) noexcept {
    if (K == 0 || (K % 256u) != 0) {
        for (std::size_t m = 0; m < M; ++m) y[m] = 0.0f;
        return;
    }
    const std::size_t blocks_per_row = K / 256u;
    const std::size_t bytes_per_row  = blocks_per_row * 210u;

    float scratch[256];
    for (std::size_t m = 0; m < M; ++m) {
        const std::uint8_t* row = w + m * bytes_per_row;
        float acc = 0.0f;
        for (std::size_t b = 0; b < blocks_per_row; ++b) {
            dequant_q6k_block(row + b * 210u, scratch);
            const float* xslice = x + b * 256u;
            for (std::size_t i = 0; i < 256; ++i) acc += scratch[i] * xslice[i];
        }
        y[m] = acc;
    }
}

// AVX2: per 16-elt sub-block, unpack the 6-bit signed values into a 16-elt
// f32 scratch, do 2×AVX2 FMA against x, hsum, then apply (d * scale_s) once.
// No per-element min term (Q6_K is symmetric around zero after the -32 shift).
void matvec_q6k_f32_avx2(const std::uint8_t* w, const float* x, float* y,
                         std::size_t M, std::size_t K) noexcept {
    if (K == 0 || (K % 256u) != 0) {
        for (std::size_t m = 0; m < M; ++m) y[m] = 0.0f;
        return;
    }
    const std::size_t blocks_per_row = K / 256u;
    const std::size_t bytes_per_row  = blocks_per_row * 210u;

    alignas(32) float sub_nib[16];

    for (std::size_t m = 0; m < M; ++m) {
        const std::uint8_t* row = w + m * bytes_per_row;
        float acc = 0.0f;

        for (std::size_t b = 0; b < blocks_per_row; ++b) {
            const std::uint8_t* blk = row + b * 210u;
            const std::uint8_t* ql  = blk + 0;
            const std::uint8_t* qh  = blk + 128;
            const std::int8_t*  scales = reinterpret_cast<const std::int8_t*>(blk + 192);
            const float d = read_fp16_le(blk + 208);
            const float* xblk = x + b * 256u;

            for (unsigned s = 0; s < 16; ++s) {
                const int scale = scales[s];
                for (unsigned j = 0; j < 16; ++j) {
                    const unsigned i     = s * 16u + j;
                    const unsigned low4  = (ql[i >> 1] >> ((i & 1u) * 4)) & 0x0Fu;
                    const unsigned high2 = (qh[i >> 2] >> ((i & 3u) * 2)) & 0x03u;
                    const int      q6s   = static_cast<int>(low4 | (high2 << 4)) - 32;
                    sub_nib[j] = static_cast<float>(q6s);
                }
                const float* xsub = xblk + s * 16u;

                __m256 nv0 = _mm256_load_ps (sub_nib + 0);
                __m256 nv1 = _mm256_load_ps (sub_nib + 8);
                __m256 xv0 = _mm256_loadu_ps(xsub    + 0);
                __m256 xv1 = _mm256_loadu_ps(xsub    + 8);
                __m256 acc_v = _mm256_fmadd_ps(nv0, xv0, _mm256_setzero_ps());
                acc_v = _mm256_fmadd_ps(nv1, xv1, acc_v);
                acc += d * static_cast<float>(scale) * hsum256(acc_v);
            }
        }
        y[m] = acc;
    }
}

void matvec_q6k_f32(const std::uint8_t* w, const float* x, float* y,
                    std::size_t M, std::size_t K) noexcept {
    matvec_q6k_f32_avx2(w, x, y, M, K);
}

} // namespace ultima::kernels
